...
void* process_thread(void *arguments) {
    ...
    while (1) {
        while (queues[index] == NULL) {
            ...
            pthread_cond_wait(empty_cv, cv_lock);
        ...
        if (args->out == 2) {
            ...
            pthread_mutex_lock(outputLock);
            char temp[1000] = "";
            sprintf(temp, "time=%d, cpu=%d, pid=%d, burstlen=%d, remainingtime=%d\n", timeval_diff_ms(&start, &cur_time), args->index, p->pid, p->burst_length, p->remaining_time);
            strncat(output, temp, 1000);
            pthread_mutex_unlock(outputLock);
...
int main(int argc, char *argv[]) {
    ...
    cv_lock = (pthread_mutex_t*)malloc(sizeof(pthread_mutex_t));
    empty_cv = (pthread_cond_t*)malloc(sizeof(pthread_cond_t));
    pthread_mutex_init(cv_lock, NULL);
